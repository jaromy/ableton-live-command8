...
class DetailViewControllerComponent(ControlSurfaceComponent):
    __module__ = __name__
    __doc__ = ' Component that can toggle the device chain- and clip view of the selected track. Shift functionality has been inverted from original DetailViewComponent, which was copied from APC40 code'
    ...
    def _nav_value(self, value, sender):
        assert ((sender != None) and (sender in (self._left_button,
                                                 self._right_button)))
        if self.is_enabled() and self._shift_pressed:
